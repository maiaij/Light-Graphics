...
void print_pixel_colour(std::ostream& out, color pixel_color, int samples_per_pixel) {
    auto r = pixel_color.x();
    auto g = pixel_color.y();
    auto b = pixel_color.z();

    // set N/A / null values to 0
    if (r != r) r = 0.0;
    if (g != g) g = 0.0;
    if (b != b) b = 0.0;
    ...
    // write the colors
    out << static_cast<int>(256 * bound(r, 0.0, 0.999)) << ' '
        << static_cast<int>(256 * bound(g, 0.0, 0.999)) << ' '
        << static_cast<int>(256 * bound(b, 0.0, 0.999)) << '\n';
}

// without phong - just diffused light
...
color path_ray_color(const ray& r, const color& background, const hittable& world, int depth) {
    ...
    //dotProduct = dot(scattered.get_direction(), object.get_normal());
    ...
    //return emitted + attenuation * ray_color(scattered, background, world, depth - 1); // without phong constants
...
int main() {
    ...
    int samples_per_pixel = 100; // 1000 - 500 - 100 - 50
    int max_depth = 25; // 50 - 50 - 25 - 15
